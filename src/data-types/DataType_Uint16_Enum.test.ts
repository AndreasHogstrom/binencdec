...
import DataType_Uint16_Enum from "./DataType_Uint16_Enum"

test.each([
  ["abc", new Uint8Array([0x00, 0x00]).buffer],
  ["def", new Uint8Array([0x00, 0x01]).buffer],
  ["ghi", new Uint8Array([0x00, 0x02]).buffer],
  ["unknown", new Uint8Array([0xFF, 0xFF]).buffer],
  ["", new Uint8Array([0xFF, 0xFF]).buffer],
])("Encode #%#", (a, expected) => {
  const dt = new DataType_Uint16_Enum("test_field[abc,def,ghi]")
...
test.each([
  [new Uint8Array([0x00, 0x00]).buffer, "abc"],
  [new Uint8Array([0x00, 0x01]).buffer, "def"],
  [new Uint8Array([0x00, 0x02]).buffer, "ghi"],
  [new Uint8Array([0x00, 0x04]).buffer, undefined],
  [new Uint8Array([0xFF, 0xFF]).buffer, undefined],
])("Decode #%#", (a, expected) => {
  const dt = new DataType_Uint16_Enum("test_field[abc,def,ghi]")
