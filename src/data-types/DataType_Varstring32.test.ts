...
import DataType_Varstring32 from "./DataType_Varstring32"

function _genString(length: number): string {
  let value = ""
  while (length > 0) {
    value += "A"
    length--
...
test.each([
  ...
  [_genString(130), new Uint8Array([
    0b1000_0001,
    0b0000_0010,
    ...(_genString(130).split("").map(v => v.charCodeAt(0))),
  ]).buffer],
])("Encode #%#", (a, expected) => {
